...
To automatically link to the correct version of libcunilog you can insert the following code in one of your header files, preferably the one that contains your logging code. It works with Cunilog in a static library or a shared dynamically linked library (DLL) with static import library (.lib). The latter assumes [Implicit Linking](https://learn.microsoft.com/en-us/cpp/build/linking-an-executable-to-a-dll?view=msvc-170).
```C
/*
